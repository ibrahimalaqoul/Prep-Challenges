...
function square(arr) {
    let myArray = arr.map(value => value * value);
    return myArray;
}
...
function fullName(arr) {
    let fullName = arr.map(value => `${value.firstName} ${value.lastName}`)
    return fullName;
}
...
function studentsResult(arr) {
    const myArray = arr.map((value => ({ 
        ...value,
        ...( value.avg >= 50 && { result: "Passed" }),
        ...( value.avg < 50 && { result: "Failed" }),
